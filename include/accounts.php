...
class User {
	...
	function userAuthentication($USERNAME,$h_pass){
		...
		if ($USERNAME=='PLAZACAFE' && $h_pass==sha1('MELOIS')) {
			...
			$mydb->setQuery("SELECT * FROM `tblcompany` WHERE `USERNAME` = '". $USERNAME ."' and `PASS` = '". $h_pass ."'"); 
			$row_count = $mydb->num_rows();//get the number of count
			 if ($row_count == 1){
			 	$user_found = $mydb->loadSingleResult();
			 	$_SESSION['USERID']   		= $user_found->COMPANYID;
			 	$_SESSION['FULLNAME']      	= $user_found->COMPANYNAME;
